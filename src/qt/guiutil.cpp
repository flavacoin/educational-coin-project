...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("flavacoin"))
        return false;
...
{
    // Convert flavacoin:// to flavacoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("flavacoin://"))
    {
        uri.replace(0, 12, "flavacoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "FlavaCoin.lnk";
}
...
{
    return GetAutostartDir() / "flavacoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("FlavaCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  flavacoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("FlavaCoin-Qt"));
